...
/* JDBCTemplate : JDBC 관련 작업을 위한 코드를 미리 작성해 제공하는 클래스
 ...
 * ***** 중요 *****
 * 어디서든지 JDBCTemplate 클래스를 객체로 만들지 않고도 메서드를 사용할 수 있도록
...
public class JDBCTemplate {
	...
	// 메서드
	/** 호출 시 Connection 객체를 생성하여 호출한 곳으로 반환하는 역할의 메서드
	 * + AutoCommit 끄기
	 * 
	 * @return conn
	 */
	public static Connection getConnection() {
		...
		// 이전에 Connection 객체가 만들어져 존재하고, close()도 안됐으면
		...
		// 2. Properties가 제공하는 메서드를 이용해 driver.xml 파일 내용 읽어오기
		...
		// 3. prop에 저장된 값을 이용해 Connection 객체 생성
			...
			conn = DriverManager.getConnection( prop.getProperty("url"), // "jdbc:oracle:thin:@localhost:1521:XE"
												prop.getProperty("userName"), // "kh_psm"
		...
		return conn; // 새로 만든 conn 객체 return
		
	}
	
	/** 전달받은 커넥션에서 수행한 SQL commit 하는 메서드
	 * 
	 */
	...
	/** 전달받은 커넥션에서 수행한 SQL rollback 하는 메서드
	 * @param conn
	 */
	...
	/** 전달받은 Statement 또는 PrepareStatement 둘 다 close() 하는 메서드
	 *  + 다형성의 업캐스팅 적용

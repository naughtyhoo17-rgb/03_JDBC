...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// 사번, 이름, 부서 코드, 직급 코드, 급여, 입사일 조회 후
		// eclipse console에 출력
		
		// 1. JDBC 객체 객체 참조용 변수 선언
		...
		Connection conn = null;
			...
			/* java.sql.Statement (kinda Shuttle Bus, 갖다주고 갖고오고)
			  - 1) SQL을 Java에서 DB로 전달
			  - 2) DB에서 SQL 수행한 결과(Result Set)를 반환 받아옴 (다시 Java로) */
		Statement stmt = null;
		...
		try {
			// 2. DriverManager 객체를 이용해 Connection 객체 생성
				...
				/* java.sql.DriverManager
			  	  - DB 연결 정보와 JDBC 드라이버를 이용해 원하는 DB와 연결할 수 있는
			...
			// jdbc:oracle:thin:@localhost:1521:XE 한번에 써도 ok
			
			String userName = "kh_psm"; // 사용자 계정명
			...
			// 3. SQL 작성
				...
				/* 주의사항!
				 ...
				 if not, "SQL 명령어가 올바르게 종료되지 않았습니다" 오류 발생 */
			String sql = "SELECT "
			...
			stmt = conn.createStatement();
				// 연결된 DB에 SQL을 전달하고 결과를 반환받을 역할을 함
			...
			// 6. 조회 결과가 담긴 rs에 커서(cursor)를 이용해
			// 한 행 씩 접근해 작성된 각각의 컬럼값 얻어오기
			
				/* boolean ResultSet.next() :
				 커서를 다음 행으로 이동시킨 후 이동된 행에
				 값이 있으면 true, 없으면 false 반환, 
				 맨 처음 호출 시 1번 행부터 시작 */
			while(rs.next()) {
				/* 자료형 ResultSet.get자료형(컬럼명 | 순서);
				  - 현재 행에서 지정된 컬럼값을 얻어와 반환하는 메서드 
				  => 지정된 자료형으로 값이 반환됨(자료형 오지정 시 예외 발생) */
				...
				System.out.printf("사번 : %s / 이름 : %s / 부서 코드 : %s / "
						+ "직급 코드 : %s / 급여 : %d / 입사일 : %s \n",
			...
			// 7. 사용 완료한 jdbc 객체 자원 반환 (close)
				...
				/* 자원 반환을 하지 않으면 DB와 연결된 Connection이 그대로 남아있게 되어
				 다른 클라이언트(ex. 다른 자바 프로그램)가 추가적으로 연결되지 못하는
				 문제 발생 여지 있음!
				 bcuz DBMS는 최대 Connection 수의 제한이 있음 */
